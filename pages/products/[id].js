...
export const getServerSideProps = wrapper.getServerSideProps(
  async ({ store, req, res, query }) => {
    const { data } = await Axios.get(
      `http://localhost:3000/custom-api/product-details/?id=${query.id}`
    );
    store.dispatch({ type: "SERVER_PRODUCT_LOADED", payload: data });
  }
);
...
const ProductScreen = (props) => {
  ...
  const product = useSelector((state) =>
    state.productDetails.server ? state.productDetails.server.product : {}
  );
